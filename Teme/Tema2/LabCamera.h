...
namespace Tema
{
	...
	public:
		...
		/* Daca suntem pe First Person Camera, acesta trebuie sa urmareasca 
		   jucatorul cand sare sau cade de pe harta */
		void MoveFirstInJumpOrFall(Player* player, float deltaTimeSeconds)
		{
			if (player->isFalling) {
				TranslateForward(3.0f * deltaTimeSeconds);
				TranslateUpword(-3.0f * deltaTimeSeconds);
			}
			else if (player->isJumping == true) {
				TranslateUpword(4.0f * deltaTimeSeconds);
			}
			else if (player->isGoingDown == true) {
				TranslateUpword(-4.0f * deltaTimeSeconds);
			}
			else {
				position.y = 1;
